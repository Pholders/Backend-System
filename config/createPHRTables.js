...
async function createPHRTables() {
  try {
    ...
    const healthVitalsExists = await pool.query(`
      SELECT EXISTS (
        SELECT FROM information_schema.tables 
        WHERE table_name = 'health_vitals'
      )
    `);
    
    if (!healthVitalsExists.rows[0].exists) {
      await pool.query(`
    ...
    const phrDocumentsExists = await pool.query(`
      SELECT EXISTS (
        SELECT FROM information_schema.tables 
    ...
    if (!phrDocumentsExists.rows[0].exists) {
      await pool.query(`
    ...
    const phrAccessExists = await pool.query(`
      SELECT EXISTS (
        SELECT FROM information_schema.tables 
        WHERE table_name = 'phr_access'
      )
    `);
    
    if (!phrAccessExists.rows[0].exists) {
      await pool.query(`
    ...
    const phrAccessRequestsExists = await pool.query(`
      SELECT EXISTS (
        SELECT FROM information_schema.tables 
        WHERE table_name = 'phr_access_requests'
      )
    `);
    
    if (!phrAccessRequestsExists.rows[0].exists) {
      await pool.query(`
    ...
    const phrAccessLogsExists = await pool.query(`
      SELECT EXISTS (
        SELECT FROM information_schema.tables 
        WHERE table_name = 'phr_access_logs'
      )
    `);
    
    if (!phrAccessLogsExists.rows[0].exists) {
      await pool.query(`
