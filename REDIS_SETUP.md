...
1. **On Read (findByEmail, findById)**:
   - Check Redis cache first
   - If found, return cached data (fast ✅)
   - If not found, query database and cache result
...
To extend caching to Doctor/Pharmacy/Admin:

1. Update respective models:
   ```javascript
   const cache = require('../services/cacheService');
   
   static async findByEmail(email) {
     const cacheKey = `doctor:email:${email}`;
...
2. Update in `update()` method:
   ```javascript
   await cache.del(`doctor:id:${id}`);
   await cache.del(`doctor:email:${email}`);
   ```
...
- Redis Cloud (https://app.rediscloud.com)
...
Update `cacheService.js` to use `REDIS_URL` for production deployments.
