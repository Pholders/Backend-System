...
/**
 * Admin Login
 * Saves admin credentials for system administration tests
 */
...
async function adminLogin() {
  ...
  try {
    ...
    if (response.data.success && response.data.data) {
      ...
      console.log('📧 OTP has been sent to admin email');
      console.log(`\n📁 Admin email saved to .test-data-admin.json for next steps`);
      console.log('\n👉 Next step: Run test-admin-02-verify-otp.js with the OTP code');
...
if (process.argv[2] === '--help') {
  console.log('\n═'.repeat(60));
  console.log('📋 USAGE: node test-admin-01-login.js [EMAIL] [PASSWORD]');
  console.log('═'.repeat(60));
  console.log('\nExample:');
  console.log('  node test-admin-01-login.js admin@example.com secure123');
